<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8"%>
<%@ include file="/WEB-INF/include/taglib.jsp"%>

<!DOCTYPE html>
<html>
<head>
	<meta charset="UTF-8">
	<meta name="viewport" content="width=device-width, initial-scale=1.0">
	<link rel="icon" type="image/png" sizes="32x32" href="${_ctx}/favicon.png">
	<title>양파마켓</title>
	
	<jsp:include page="${_resource_admin}"/>
</head>

<body>
	<div class="admin-page">
		
		<jsp:include page="${_footer_admin}"/>
		
		<div class="wrap">
			
			<jsp:include page="${_header_admin}"/>
			
			<main>
                <h1 class="heading1">신고관리 상세</h1>

                <div class="board-lst">
                    <!-- 사용자 신고글 -->
                    <div class="board-lst-datail type-user">
                        <span class="badge1">처리완료</span>
                        <!-- <span class="badge2">미접수</span> -->
                        <span class="type">회원 문의</span>
                        <ul>
                            <li>
                                <p class="num">물품거래 게시글 번호 : 138</p>
                            </li>
                            <li>
                                <p class="tit">이 사람 이상해서 신고해요</p>
                            </li>
                            <li>
                                <div class="info-area">
                                    <p class="id">아이디 : dditislove</p>
                                    <p class="date">신고접수일 : 2023-10-12</p>
                                    <p class="date">피드백날짜 : 2023-10-12</p>
                                </div>
                            </li>
                            <li>
                                <p class="txt">처리해주세요. 처리해주세요.처리해주세요.처리해주세요.처리해주세요.처리해주세요.</p>
                            </li>
                        </ul>
                    </div>

                    <!-- 관리자 답변글 -->
                    <div class="board-lst-datail type-admin">
                        <span class="type">관리자 답변</span>
                        <ul>
                            <li>
                                <p class="tit">접수하신 신고가 처리 완료되었습니다.</p>
                            </li>
                            <li>
                                <div class="info-area">
                                    <p class="id">관리자</p>
                                    <p class="date">답변일 : 2023-10-12</p>
                                </div>
                            </li>
                            <li>
                                <p class="txt">해당 게시글의 사용자를 정지 처리 하였습니다.</p>
                            </li>
                        </ul>
                        <div class="btn-area">
                            <input type="button" value="수정하기" class="btn-style4">
                            <input type="button" value="삭제하기" class="btn-style5">
                        </div>
                    </div>
                </div>
            </main>
			
		</div>
		
	</div>

</body>
</html>